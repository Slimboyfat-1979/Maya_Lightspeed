...
function display_products() {
    ...
    while($row = mysqli_fetch_assoc($query)) {
        ...
        <td>{$row['product_title']}</td>
        <td><a href="index.php?edit_product&id={$row['product_id']}"><img style="width:100px; height=100px; object-fit: cover;" src="../assets/images/products/{$row['product_image']}" alt=""></a></td>
        <td>{$category_id}</td>
        <td>{$row['product_price']}</td>
        <td>{$row['product_qty']}</td>
        <td><a class="btn btn-danger" href="../resources/templates/back/delete_product.php?id={$row['product_id']}"><span>&times;</span></a></td>
        </tr>
DELIMETER;
        echo $products;
    }
